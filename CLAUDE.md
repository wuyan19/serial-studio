...
- **DeviceClientManager**(server/src/device/):每注册设备一条 WS(读 remotes.json,eager 连接,退避重连,协议层 Ping 心跳 10s/20s,断连积压命令在新会话开始时丢弃),挂 AppState 跨热重启保留,`start()` 惰性幂等(supervisor.start 触发)。断连=USB 拔插:RemotePortIo read 返 Err → drainer 标 disconnected 保占有权 → reopen 走新连接。**断链逐跳传播**:远端单端口断开(远端 Disconnected 事件)→ 本侧同线名 IO `close_with_error` 注入 → 本侧 drainer 标 disconnected——中间跳掉线(A→B→C 的 C 断)与远端本地口 USB 拔出,下游各跳都能感知,不再 RX 静默断流。**防环=段重复环检测 + 透传深度上限**(谓词 `passthrough_port_key`,lib.rs,列表合并与别名索引共用):远端桶多级条目(`devB::devC::COM3`)透传,候选键段链有重复(自连第二轮 `me::me::COM7`、互注册绕回 `ab::ba::ab::…`)或段数超上限(≤4)即丢弃——否则自连/互注册时列表每轮上报自我复制(回声环,4→8→12…直至消息风暴拖死进程)。自连(把本机地址注册为远程设备)因此成为合法形态:本地口的远程镜像,open 路由回本机(附加到同一端口的占有权)。互注册时每侧稳定保留一条**幽灵条目**(自己口经对方绕回,段链无重复检测不到,有界无害)。多级级联的口在直连设备的 UI 可见(归其分组平铺,`displayPortName` 只显串口名)。
...
- **端口键是"视角路径"而非全局端口 id**——同一物理口在各机键不同(A 视角 `ab::bc::COM7`、B 视角 `bc::COM7`);禁止按键跨机器去重/比较。**段名是各机 remotes 的本地命名,中间机器无法解析他人段名**——环检测只能用结构性判据(段重复),这是未来引入全局实例 id(path-vector)的接缝:谓词 `passthrough_port_key` 可整体替换,升级时勿在别处另起炉灶。
